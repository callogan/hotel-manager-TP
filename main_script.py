...
def calculate_total_prices(data):
    ...
    Returns:
        list of tuple: List, containing the information about the room, the net price and total price with taxes.
    """
    total_prices = []
...
def display_results(min_price_room, min_price_value, number_of_guests, total_prices):
    """
    Forms the string for reflecting results about minimal price and total rice all the rooms.
    ...
    result = []
    result.append(
        f"The lowest price: {min_price_value} USD, room type: {min_price_room}, number of guests: {number_of_guests}\n")

    result.append("Total price for all the rooms:")
    for room, price, total_price in total_prices:
        result.append(f"Room: {room}, price: {price} USD, total price (with taxes): {total_price:.2f} USD")
...
def main():
    ...
    """
    file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'hotel_data.json')
    ...
    result = display_results(min_price_room, min_price_value, number_of_guests, total_prices)
